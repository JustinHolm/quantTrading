...
sell_from_long = 0.0005

buy_from_short = -0.0005
...
for e in exchanges:
    for pair in pairs:
        ...
        df_ohlcv['Lx'] = df_ohlcv['low'].rolling(x).min() 
        df_ohlcv['Hx'] = df_ohlcv['high'].rolling(x).max() 

        df_ohlcv['Ly'] = df_ohlcv['low'].rolling(y).min() 
        df_ohlcv['Hy'] = df_ohlcv['high'].rolling(y).max() 

        df_ohlcv['OSC_signal_x'] = ((df_ohlcv['close'] - df_ohlcv['Lx'])/(df_ohlcv['Hx']-df_ohlcv['Lx'])) /100
        df_ohlcv['OSC_signal_y'] = ((df_ohlcv['close'] - df_ohlcv['Ly'])/(df_ohlcv['Hy']-df_ohlcv['Ly'])) /100

        df_ohlcv['OSC_signal'] = df_ohlcv['OSC_signal_y'] - df_ohlcv['OSC_signal_x']
        ...
        last_row = df_ohlcv[['OSC_signal','close','RSI_10m','RSI_60m','EWMA','trade','buy_at','sell_at','signal','signal_EWMA','signal_RSI','high','low']].tail(1)

        close = float(last_row['close'].values[0])
        EWMA = float(last_row[['EWMA']].values[0])
        RSI = float(last_row[['RSI_60m']].values[0])
    
        signal_OSC =  last_row['OSC_signal'].values[0]
        ...
        signal = (1 * signal_OSC) + (0 * signal_EWMA) + (0 * signal_RSI) + (0 * signal_OB) + (0 * signal_trade_imb)
